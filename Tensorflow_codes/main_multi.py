...
def func(_args):
	...
	print(f'{arr_rate_},{version},{train_sim}')
	...
	#actor_weights_itr_400.h5
	#train_iter = 400
	directory = f"../data/merged_replay_buffer_with_next_state/train_sim_{train_sim}/trained_weights/actor_weights_itr_{train_iter}"  
	#directory = f"../data/merged_replay_buffer_with_next_state/train_sim_{train_sim}/trained_weights"  
	os.makedirs(directory, exist_ok=True)
 
	#print(os.path.isdir(directory))
	filename = f"actor_weights_itr_{int(train_iter)}.h5"
	file_path = os.path.join(directory, filename)
	#exit()
	#print(f'file check: {os.path.isfile(file_path)}')
	#exit()

 	#assert os.path.isdir("~/mowbrayr/New Folder/Framework_rl_signal/data/merged_replay_buffer_with_next_state/train_sim_{train_sim}/trained_weights"),f"DIR"
	#assert os.path.isfile("../data/merged_replay_buffer_with_next_state/train_sim_{train_sim}/trained_weights/actor_weights_itr_{train_iter}.index"),f'file ERROR'
	
	if (not learning_flag): agent.model.load_weights(file_path)		
	############## streamline ######################
		...
		for rep_sim in range(0, max_rep_sim): # through DDPG it will 61 episodes
			...
			while (time_track < data_file.max_sim_time):  
				...
				if data_file.rl_algo_opt == "DDPG" and learning_flag and (agent.buffer.buffer_counter > 0) and ((time_track % 1) == 0):
					...
					#print(f'actor*****weights:{agent.model.actor_model.variables}, \n\n tar_weights:{agent.target_model.actor_model.variables}')
					#print(f'crtic*****weights:{agent.model.critic_model.variables}, \n\n tar_weights:{agent.target_model.critic_model.variables}')
					#print(f'model*****weights:{agent.model.variables}, \n\n tar_weights:{agent.target_model.variables}')
					#exit()
					...
					#agent.update_target(agent.target_model.actor_model.variables, agent.model.actor_model.variables, agent.tau_)

				if learning_flag and (time_track % 100) == 0:
					directory = f"../data/version_{version}/arr_{arr_rate_}/train_homo_stream/train_sim_{sim}/sim_data/trained_weights/"  ####********
					os.makedirs(directory, exist_ok=True)
					filename = f"actor_weights_itr_{int(time_track)}.weights.h5"
					file_path = os.path.join(directory, filename)
				...
				if (algo_option == "rl_modified_ddswa") and (learning_flag):
					if (not (prev_rew == None)) and (not (len(prev_state) == 0)):
						...
						if rep_sim%10==0 and time_track == data_file.max_sim_time :
							dbfile = open(f'../data/version_{version}/arr_{arr_rate_}/train_homo_stream/train_sim_{sim}/replay_buffer_sim_{sim}','wb')
							pickle.dump(qwe, dbfile,protocol = 4) #pickle.HIGHEST_PROTOCOL)
							#with open('../data/arr_{arr_rate_}/train_homo_stream/train_sim_{sim}/replay_buffer_sim_{sim}.yml', 'w') as outfile:
							#	yaml.dump(data, outfile, default_flow_style=False)
							dbfile.close()
							wr_coun =0
							#### compress after pickle ######
							#pickled_data = cloudpickle.dumps(qwe)  # returns data as a bytes object
							#compressed_pickle = blosc.compress(pickled_data)
							#with open(f'../data/arr_{arr_rate_}/train_homo_stream/train_sim_{sim}/replay_buffer_sim_{sim}','wb') as f:
				...
				if not data_file.run_coord_on_captured_snap:
					...
					for _train_iter in _train_iter_list:
						for _sim_num in range(1, 11):  #11
							...
							for _train_sim in list(range(1,11)):   ##### edited############### 11 $#############################
								for _arr_rate_ in arr_rates_to_sim:
									arr_rate_array_ = {0:0, 1:_arr_rate_, 2:_arr_rate_, 3:0, 4:_arr_rate_, 5:_arr_rate_, 6:0, 7:_arr_rate_, 8:_arr_rate_, 9:0, 10:_arr_rate_, 11:_arr_rate_} #arr_rate*np.ones(len(lanes)) #
