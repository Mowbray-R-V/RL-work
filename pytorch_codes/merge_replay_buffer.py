...
##################################################################################################
arr_rate_array = [0.1]#data_file.arr_rates_to_simulate #[0.11, 0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18, 0.19, 0.2]
################################################################################################################
...
for train_sim in range(1,2):
	...
	for ind, arr_rate in enumerate(arr_rate_array):
		...
		print(f"type:{type(buffer)}, size_buff:{sys.getsizeof(buffer)}")
		print(f" value: {type(buffer['reward_buffer'])}, size:{np.size(buffer)}, shape:{np.shape(buffer)}  ")
		# exit()
		...
		print('mrb',merged_replay_buffer["state_buffer"][individual_buffer_size*ind: individual_buffer_size*(ind + 1)].shape)
