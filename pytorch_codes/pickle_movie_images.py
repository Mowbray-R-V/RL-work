...
def plot_ellipse(veh_object, veh_id, center_pos_on_lane, lane, tc_flag, dict, flag=False):
...
def func(heuristic):
    ...
    if heuristic == "RL":
        file_path = f"./data/test_homo_stream/arr_{arr_}/train_sim_1/train_iter_12500/pickobj_sim_{sim_}"
    else:
        file_path = f"./data/{heuristic}/arr_{arr_}/pickobj_sim_{sim_}"

    for c in os.listdir(f"{file_path}"):
        print('dafsgfsgdfg',os.listdir(f"{file_path}"))
        print(f"{file_path}/{c}")
