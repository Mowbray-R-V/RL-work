...
def func(_args):
		...
		if data_file.rl_algo_opt == "DDPG":
			if algo_option == "rl_modified_ddswa":
				...
				if learning_flag:         
					...
					curr_obs = None
					prev_obs = None
					max_rep_sim = 101 # for CML 61 episodes of data, each of 500 sec [data collection phase]
	...
	if (not learning_flag): 
      #f"../data_version/version_{int(version)}/arr_{arr_rate_}/train_homo_stream/train_sim_{sim}/sim_data/trained_weights/model_weights_itr_{int(time_track)}" 
		# directory = f"./data/train_homo_stream/train_sim_{sim}/sim_data/trained_weights/model_weights_itr_{train_iter}"
		...
		filename = 'actor_weights_itr_400.weights.pth'
		...
		for rep_sim in range(0, max_rep_sim): # through DDPG it will 61 episodes
			...
			while (time_track < (data_file.max_sim_time)+1):  
				...
				if learning_flag and (time_track % 100) == 0:
					directory = f"./data/train_homo_stream/train_sim_{sim}/sim_data/trained_weights/model_weights_itr_{int(time_track)}"  
					os.makedirs(directory, exist_ok=True)
				...
				if functions.get_num_of_objects(sim_obj.spawned_veh) > 0 : 
						...
						if len(data_file.incompdict[lane])>0:
							...
							if dict_sig[lane]=='G':
								...
								for iter in range(n):
									...
									if not learning_flag:functions.storedata(v, train_sim, sim, train_iter,arr_rate_) 							
					...
					############# update veh and REF dict  with signal and time  ##################
				#print(f"[cont query]: controltime: {round(time.time() - control_time, 3)}")
				...
				#print(f"[buffer query]: buffer time: {round(time.time() - buff_time, 3)}")
						...
						for _sim_num in range(1, 2):  #11
							...
							for _train_sim in list(range(1,2)):   ##### edited############### 11 $#############################
								# for _arr_rate_ in arr_rates_to_sim:
									...
									file_path = f"./data/test_homo_stream/arr_{_arr_rate_}/train_sim_{_train_sim}/train_iter_{_train_iter}/sim_{_sim_num}/sim_{_sim_num}_train_iter_{_train_iter}.png"
