...
########################################### should Modularise #################################
    ...
    def forward(self, inputs):
        ...
        # print('output',output.shape)
        latent = torch.cat([hidden_forward, hidden_backward], dim=-1)
        latent=self.fc1(latent)
        # print('poutput',packed_output)

        return output, latent,lengths
...
class ActorModel(nn.Module):
    ...
    def forward(self, obs, latent):
        ...
        signal_cop =signal.repeat(1,1,data_file.num_veh)
        signal_cop=signal_cop.permute(0,2,1)
        state = latent.repeat(1,data_file.num_veh,1)
        # print('state',state.shape)
        # print('s_c',signal_cop.shape)
        # print('obs',obs.shape)
        # exit()
        input=torch.cat((state,obs,signal_cop),2)
...
class CriticModel(nn.Module):
    def __init__(self, lat_size):
        ...
        self.gru_action = nn.GRU(lat_size, 32,num_layers=1, batch_first=True)
    ...
    def forward(self, latent_state, action,length):
        action = action
        ...
        # norm_action=norm_action.squeeze(dim=1)
        # print('n_a',norm_action.size())
        

        # print(len(length))
        act=torch.zeros(64,124)

        # print(norm_action[0,0:(12+length[1])])
        for i in range(len(length)):
            a=(norm_action[i,0:(12+length[i])])
            padding_needed=124-(12+length[i])
            padding_tuple=(0,padding_needed)
            act[i,:]=F.pad(a,padding_tuple,mode='constant',value=-1)
        act=torch.FloatTensor(act).unsqueeze(dim=-1)
        # print('a0',act.size())
        action_lengths=12+length
        # Pack padded sequence
        packed_action = pack_padded_sequence(act, action_lengths, batch_first=True, enforce_sorted=False)
        # print(packed_action)
        # # GRU forward
        packed_latent_act, _ = self.gru_action(packed_action)
        # print('pla',packed_latent_act)
        ...
        latent_act=torch.sum(latent_act,dim=1)
        ...
        # exit()
        Q_val = F.relu(self.fc4(concat))
        Q_val = F.relu(self.fc5(Q_val))
        Q_val = F.relu(self.fc6(Q_val))
...
class NNModel(nn.Module):
    def __init__(self, lat_size):
        ...
        self.critic_model = CriticModel(1)
    ...
    def forward(self, input, action=None, act_or_cri='act'):
        ...
        seq,latent,length = self.RNN(input)
        ...
        if act_or_cri == 'act':
            ...
            # print('a',action.size())
            # print('a',action)
            return self.critic_model(latent, action,length)
...
class Buffer:
    def __init__(self, buffer_capacity=700000, batch_size=64, state_size=None, observe_size=None, action_size=5, buff_model=None, buff_model_target=None, gamma=0.99, tau=0.001, critic_optimizer=None, actor_optimizer=None):
        #######################################################################
        self.state_size =data_file.num_veh*15
        ########################################################################
        self.action_size = data_file.num_veh + data_file.num_phases
...
class DDPG:
    def __init__(self, sim=0, noise_mean=0, noise_std_dev=0.2, cri_lr=0.001, act_lr=0.0001, disc_factor=0.99, polyak_factor=0.001, buff_size=1000, samp_size=64):
        ...
        self.num_states = data_file.num_features*data_file.num_veh
        ...
        self.model = NNModel(32)#.to(device) 64 should be inside the ()
        self.target_model = NNModel(32)#.to(device)  64 should be inside the ()
    ...
    def policy(self, state, noise_object, num_veh=None):
        ...
        assert not state_has_nan, f'input values-bad : state: {state}'
