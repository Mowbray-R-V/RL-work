...
logging.basicConfig(filename='RES_violation.log', level=logging.INFO)
    ...
    if _pre_v == None:
        ...
        if len(veh.p_traj) >=1:   
            ...
            assert _pre_v.p_traj[pre_ind_nxt] -  x_prime_z[0] >= data_file.L + max(0,((_pre_v.v_traj[pre_ind_nxt]**2 - x_prime_z[1]**2 )/(2*veh.u_min))) , \
                f'prev_pos:{_pre_v.p_traj[pre_ind_nxt] }, prev_vel:{_pre_v.v_traj[pre_ind_nxt]},  curr_pos:{ x_prime_z[0]},curr_vel:{ x_prime_z[1]},  RHS:{data_file.L + max(0,((_pre_v.v_traj[pre_ind_nxt]**2 - x_prime_z[1]**2 )/(2*veh.u_min)))}, \
