...
def tf_to_np(tensor):
    num = np.array(tensor)
    tf.print(f'inisde funv****:{num},{num.shape}')
    #`exit()
    return num
...
class SeqModule(layers.Layer):
    ...
    def call(self, inputs):
        ...
        #print(f'{non_padded_indices},{}')
        
        
        print(f'index:{index},{index.shape}')

        # Expand the index to match the batch size
        batch_size = tf.shape(seq_output)[0]
        print(f'batch_size:{batch_size},{batch_size.shape}')
        ...
        latent = tf.gather_nd(seq_output, indices)
        print(f'extracted_features:{latent},{latent.shape}')
...
class NNModel(tf.keras.Model): 
    ...
    def call(self, input, action=None, act_or_cri='act'):
        ...
        if act_or_cri == 'act': 
            ...
            tf.print(f"recieved feed from act,{y_hat}")
            return y_hat
        elif act_or_cri == 'critic': 
            tf.print("recieved feed from critic")
...
class DDPG:
    ...
    def __init__(self, sim=0, noise_mean=0, noise_std_dev=0.2, cri_lr=0.001, act_lr=0.0001, disc_factor=0, polyak_factor=0, buff_size=1000, samp_size=64):
        ...
        #print(f'build ouptput:   {self.model(tf.random.uniform((64, self.num_states)), act_or_cri='act')}')
        buid_ouput = self.model(tf.random.uniform((64, self.num_states)), act_or_cri='act')##
        print("buid/********************0")
        buid_ouput_1 = self.model(tf.random.uniform((64, self.num_states)), tf.random.uniform((64, self.num_actions)),act_or_cri='critic')
        print("buid---critic/********************0")
        exit() 
        #print(_)
        #exit()
